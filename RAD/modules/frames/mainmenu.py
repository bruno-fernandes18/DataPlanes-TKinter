...
class MainMenu:
    def __init__(self,root: tk.Tk, user, plane_callback, creator_callback, manager_callback, register_callback, login_callback, logoff_callback):
        try:
            ...
            self.btn_right = MenuButton(self.frm_account, 'Login', self.open_login).AccountButton('right') if self.user == 'Not Logged!' else MenuButton(self.frm_account, 'Logoff', self.do_logoff).AccountButton('right')
    ...
    def open_register(self):
        ...
        AccountMenu(self.root, self.reenable_buttons, self.do_regis).open_tab(False)
    def open_login(self):
        self.disable_buttons()
    ...
    def reset_buttons(self):
        ...
        self.btn_right = MenuButton(self.frm_account, 'Login', self.open_login).AccountButton('right') if self.user == 'Not Logged!' else MenuButton(self.frm_account, 'Logoff', self.do_logoff).AccountButton('right')
        self.btn_left = MenuButton(self.frm_account, 'Register', self.open_regis).AccountButton('left') if self.user == 'Not Logged!' else None
